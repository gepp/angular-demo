...
app // 修改
    .controller('questionEditCtrl', ["$scope", "$http", "$state", "$stateParams", 'Tag', 'Indicator', '$modal', function($scope, $http, $state, $stateParams, Tag, Indicator, $modal) {
    ...
    var __DEFAULT_QUE__ = { // 默认题目数据
        stem: { // 题干 
            content: "如果A、B、C、D、E表示干一番事业的愿望程度，你会选择：（）",
            photos: ["/images/b1.jpg", "/images/b2.jpg", "/images/b3.jpg", "/images/b4.jpg", "/images/b5.jpg"]
        }
    };
    $scope.question = angular.copy(__DEFAULT_QUE__);

    $scope.$watch('question.stem.photos', function(_new, old, scope) {
        console.log($scope.question.stem.photos);
    },true);
    ...
    $scope.openSelect = function() {
        var modalInstance = $modal.open({
            ...
            resolve: {
                items: function() {
                    return $scope.question.indicators;
                }
            }
        });
        modalInstance.result.then(function(result) {
            $scope.question.indicators = result;
        });
    };
